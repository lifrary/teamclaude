...
```bash
# Install this customized fork rather than replacing its routing with npm upstream.
npm install -g github:lifrary/teamclaude
...
Config is at `~/.config/teamclaude.json` (`$XDG_CONFIG_HOME` honoured) and is meant to be hand-editable. A proxy API key is generated on first use.
...
### Customized routing and maintenance

This fork keeps explicit account order ahead of use-or-lose ordering: an unset
priority sorts after every finite priority, then the soonest weekly reset,
soonest session reset and lowest session utilization break ties. Model-scoped
weekly exhaustion blocks only the affected model, not the account's other models.

Per-account concurrency is bounded (`maxConcurrentPerAccount`, default 3), as
are overflow depth and buffered bodies. `overflowQueueTimeoutMs: null` waits for
healthy capacity until the client disconnects, without the predispatch deadline
expiring that queue wait; finite values time out and `0` fails fast. This does
not bypass quota or authentication failures. Retry backoffs release their slots
before sleeping. An upstream 529 does not trigger fleet-wide retries;
`overloadFallbackModel` optionally retries an overloaded Opus request once on
the same account with another model.

The maintenance coordinator serializes per-account background work. Zero-spend
usage probes can monitor accounts refused inference admission, including
disabled accounts with valid credentials, without increasing in-flight caps.
Disabled-account probes never rotate credentials; the separate periodic token
keep-alive sweep preserves idle OAuth lineages. Spending warm-ups remain gated
and do not run immediately on interval-mode startup.
...
Upstream TeamClaude is maintained at https://github.com/KarpelesLab/teamclaude
and published as [`@karpeleslab/teamclaude`](https://www.npmjs.com/package/@karpeleslab/teamclaude).
This repository is the customized lifrary fork. Installing upstream directly
does not include the custom behavior described above. See [SECURITY.md](SECURITY.md)
for upstream security reporting guidance.

...
| `s` | Switch active account (or pin a selected model route) |
| `e` | Enable / disable the selected account |
| `o` | Reorder the selected account (`↑`/`↓` move; `a` clears all ranks; `c` clears its rank) |
| `p` | Refresh quota on all accounts (one-shot zero-spend probe) |
| `R` | Reload accounts from config and force a fleet quota re-measure |
...
The same gate handles **rate-limit 429s** (the per-minute throttle, not quota exhaustion): teamclaude pauses the account for the `retry-after` window so new queries wait instead of piling on, then releases the held queries through a fresh ramp (staggered, not all at once). It **never rotates** on a rate-limit 429 — that would just move the burst to the next account and drop the first account's cache. Short waits (15 seconds or less) are absorbed inline on the same account; longer waits return a 429 + `retry-after` so the client backs off. Only a **quota rejection** (`unified-…-status: rejected`) rotates.
...
| `accounts[].priority` | Optional explicit rotation preference: lower finite values are preferred; absent or `null` means auto/unranked |
...
Any Anthropic-compatible API can be added as an account alongside your Claude accounts. Give it a higher explicit `priority` value (lower = preferred, so use e.g. `100`) while assigning Claude accounts lower finite values; it will then be used as a fallback when those Claude accounts are exhausted.
...
2. The proxy selects the active account and forwards requests with that account's credentials; its concurrency slot remains held until the full upstream response stream is forwarded or the client disconnects
3. OAuth tokens expiring within 5 minutes are automatically refreshed and persisted to config
4. Rate limit headers from the API (`anthropic-ratelimit-unified-*`) track session (5h), weekly (7d), and model-family-scoped weekly windows such as `7d_oi` (Fable). A spent model-family window makes an account ineligible only for requests in that family; it remains eligible for other models.
5. **Cold-start warm-up**: quota is only known after a request flows through an account, so at startup the proxy first routes requests to any unmeasured account until every account has been measured once. An **active warm-up** additionally probes unmeasured idle accounts directly — a minimal 1-token request reusing the shape of the first real request — so the whole fleet is measured within seconds. It sends at most one extra background probe per eligible account and never takes a client concurrency slot (`activeWarmup: false` disables it). Then account selection becomes **use-or-lose**: among accounts still under the threshold, it prefers the one whose weekly (7d) quota resets soonest (tie-breaks: soonest session reset, then lowest usage), so quota about to renew unused is drained first. Explicitly ranked accounts (`priority` / TUI `o`) are preferred before all of that; disabled accounts are excluded entirely. The active account stays sticky to keep its prompt cache warm; priority is re-evaluated every `reevalIntervalMs` (default 5 min; set `0` to disable timer-based switching), and on reaching the threshold it switches immediately to the next-highest-priority account.
6. On a 429 the proxy classifies it:
   - **Account-wide quota exhaustion** (upstream reports a spent 5h or 7d bucket) → throttles that account for its `retry-after` (clamped to 1s–5m) and immediately fails over to another account. If every account is throttled, it returns 429 with a computed `retry-after`.

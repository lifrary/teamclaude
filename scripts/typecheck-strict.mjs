...
function countAt(dir) {
  ...
  // Parser failures prevent the semantic pass, so a lower diagnostic count
  // in that case is not an improvement and must never satisfy the ratchet.
  if (/\berror TS1\d{3}:/.test(tsc.stdout)) {
    console.error(tsc.stdout);
    throw new Error(`Syntax diagnostics prevent strict baseline comparison in ${dir}`);
  }

...
test('path 3: a knob-off quota summary preserves the reset for routing', () => {
  // Snapshot reads never rotate the fleet, even with expiry routing disabled.
  ...
  am.getQuotaSummary();
  assert.equal(am.accounts[1].sessionResetPending, true,
    'the snapshot must preserve the pending routing event');
  assert.equal(am.accounts[am.currentIndex].name, 'cur',
    'reading a snapshot must not rotate the fleet');
  am.refreshExpiredQuotas(OPUS, asRequest());
  assert.equal(am.accounts[am.currentIndex].name, 'reset',
    'the routing path still consumes the pending reset');
});

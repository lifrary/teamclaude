...
test('SxManager rejects an invalid mode before mutating state or provisioning', T, async () => {
  const sx = new SxManager();
  const before = sx.mode;
  await assert.rejects(() => sx.configure('KEY', 'typo'), /Invalid sx mode/);
  assert.equal(sx.mode, before);
  assert.equal(sx.apiKey, null);
  await assert.rejects(() => sx.setMode('invalid'), /Invalid sx mode/);
  assert.equal(sx.mode, before);
});

...
test('renderStatus places unset priorities after explicit zero and names them auto', () => {
  const status = sampleStatus();
  const base = status.accounts[0];
  status.accounts = [
    { ...base, name: 'unset', priority: null },
    { ...base, name: 'last', priority: 4 },
    { ...base, name: 'first', priority: 0 },
  ];
  const output = renderStatus(status, { color: false, now });
  assert.ok(output.indexOf('first (') < output.indexOf('last ('));
  assert.ok(output.indexOf('last (') < output.indexOf('unset ('));
  assert.match(output, /unset \(oauth, prio auto\)/);
});

test('renderStatus uses per-bucket thresholds for family verdicts and weekly recovery', () => {
  const status = sampleStatus();
  status.switchThreshold = { default: 0.98, unified5h: 0.9, unified7dFable: 0.605 };
  status.accounts[0].quota = {
    unified5h: 0.8,
    unified7d: 0.7, unified7dReset: now + 7200_000,
    unified7dFable: 0.65, unified7dFableReset: now + 3600_000,
    unified7dSonnet: 0.7, unified7dSonnetReset: now + 3600_000,
  };
  const output = renderStatus(status, { color: false, now });
  assert.match(output, /Switch at\s+default:98% unified5h:90% unified7dFable:60\.5%/);
  const models = output.split('\n').find(line => line.includes('Models'));
  assert.match(models, /Opus ✓/);
  assert.match(models, /Sonnet ✓/);
  assert.match(models, /Fable ✗ 1h/, 'only the family bucket is over its own threshold');

  status.accounts[0].quota.unified7dFable = 0.5;
  const allowed = renderStatus(status, { color: false, now }).split('\n').find(line => line.includes('Models'));
  assert.match(allowed, /Fable ✓/, 'shared spend above the family limit is allowed below its own limit');

  status.switchThreshold.unified7d = 0.7;
  status.switchThreshold.unified7dFable = 0.9;
  const sharedBlocked = renderStatus(status, { color: false, now }).split('\n').find(line => line.includes('Models'));
  assert.match(sharedBlocked, /Fable ✗ 2h/, 'shared exhaustion still blocks a family with a looser limit');
  assert.doesNotMatch(sharedBlocked, /✓/);

  status.switchThreshold.unified7dFable = 0.5;
  status.accounts[0].quota.unified7dFableReset = now + 10800_000;
  const bothBlocked = renderStatus(status, { color: false, now }).split('\n').find(line => line.includes('Models'));
  assert.match(bothBlocked, /Fable ✗ 3h/, 'both independent weekly blockers must reset');

  status.switchThreshold.unified7d = 0.98;
  status.switchThreshold.unified7dFable = 0.9;
  status.switchThreshold.unified5h = 0.8;
  const blocked = renderStatus(status, { color: false, now }).split('\n').find(line => line.includes('Models'));
  assert.doesNotMatch(blocked, /✓/, 'the shared session threshold blocks every family at equality');
});

test('renderStatus displays canonical modelWeekly Fable and preserves the benched verdict safely', () => {
  const status = sampleStatus();
  const account = status.accounts[0];
  account.quota.modelWeekly = { '7d_oi': { utilization: 0.35, reset: now + 3600_000 } };
  account.available = false;
  account.benchedReason = 'disabled\x1b[31m\ninjected';
  const output = renderStatus(status, { color: false, now });
  assert.match(output, /Fable\s+\[[^\]]+\] 35% reset 1h/);
  const models = output.split('\n').find(line => line.includes('Models'));
  assert.match(models, /Fable ✗/);
  assert.doesNotMatch(models, /✓|\x1b/);
});

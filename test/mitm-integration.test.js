...
test('MITM h2: routes advisor/session context once and sanitizes orphan tool pairs upstream', T, async () => {
  const { caCertPem, leafCertPem, leafKeyPem } = generateCertChain('localhost');
  ...
  const am = new AccountManager([
    oauthAccount('A', 'TOK-A'),
    oauthAccount('B', 'TOK-B'),
  ], 0.98);
  am.accounts[0].quota.unified7dFable = 0.999;
  am.accounts[0].quota.unified7dFableReset = Date.now() + 3600_000;
  const calls = { begin: 0, record: 0, end: 0 };
  for (const method of ['beginSession', 'recordSession', 'endSession']) {
    const original = am[method].bind(am);
    am[method] = (...args) => {
      calls[method.slice(0, -7)]++;
  ...
  try {
    const client = http2.connect('https://localhost', { createConnection: () => tlsSock });
    const req = client.request({
      ':method': 'POST', ':path': '/v1/messages', authorization: 'Bearer FAKE',
      'content-type': 'application/json', 'x-claude-code-session-id': sessionId,
    });
    req.resume();
    req.end(body);
    await once(req, 'close');
    ...
    assert.equal(upstreamAuth, 'Bearer TOK-B', 'advisor model excludes A');
    assert.deepEqual(upstreamBody.messages, [{ role: 'user', content: 'continue' }]);
    assert.deepEqual(calls, { begin: 1, record: 1, end: 1 });
    assert.equal(am.sessionStats().known, 1);
    assert.equal(am.sessionStats().active, 1, 'recently completed session remains active within the TTL');
    assert.equal(am.sessionTracker.pinnedAccount(sessionId, am._weeklyBucketFor('claude-opus-4-8')), 1);
    assert.equal(am.sessionTracker.pinnedAccount(sessionId, am._weeklyBucketFor('claude-fable-5')), null,
      'routing the executor does not claim an advisor-family pin');
  } finally {
    tlsSock.destroy(); closeHard(proxy); closeHard(upstream);
  }
});

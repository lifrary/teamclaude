...
test('route rows read the shape a real AccountManager reports', () => {
  ...
  // Operator disabling immediately reselects an eligible account in this fork;
  // the dashboard must show that new cursor rather than the disabled account.
  am.setDisabled(0, true);
  const after = routeRows(am.getStatus())[rows.length - 1];
  assert.deepEqual({ target: after.target, current: after.current, why: after.currentUnavailable }, { target: 'b', current: 'b', why: null });
});

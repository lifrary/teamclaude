...
test('a Codex account with no credentials still takes them from the CLI file', async () => {
  await withTmp(async dir => {
    ...
    const [acct] = await resolveAccounts({
      accounts: [{
        id: 'stable-entry', name: 'first@example.com', type: 'oauth', provider: 'codex', importFrom: path,
        disabled: true, priority: 2, maxConcurrent: 1,
      }],
    });
    ...
    assert.equal(acct.id, 'stable-entry');
    assert.equal(acct.disabled, true);
    assert.equal(acct.priority, 2);
    assert.equal(acct.maxConcurrent, 1);
  });
});

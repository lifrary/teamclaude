...
test('distribution on: explicit priority outranks unranked accounts for new sessions', () => {
  const am = new AccountManager([
    oauth('unranked'),
    oauth('fallback', { priority: 100 }),
  ], 0.98, { distributeSessions: true });

  assert.equal(
    am.getActiveAccount(null, null, null, 'sess-1').name,
    'fallback',
    'any explicit finite priority outranks the unranked tier',
  );
});

test('distribution on: explicit priority preempts a pinned unranked session', () => {
  const am = new AccountManager([
    oauth('unranked'),
    oauth('fallback', { priority: 100 }),
  ], 0.98, { distributeSessions: true });
  ...
  assert.equal(
    am.getActiveAccount(null, null, null, 'sess-1').name,
    'fallback',
    'an explicit priority account preempts an unranked session pin',
  );
});
...
// Distribution is opt-in. Once bounded quota discovery is complete, disabling
// distribution keeps untagged requests on the measured current account.
test('distribution off: measured untagged requests stay on the current account', () => {

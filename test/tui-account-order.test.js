...
test('selection walks display positions while anchoring the manager account object', () => {
  ...
  assert.equal(tui._selected(), am.accounts[2]);
  assert.equal(tui._selected().name, 'claude-b');

  tui._key('down');
  assert.equal(tui.selIdx, 2);
  assert.equal(tui._selected(), am.accounts[1]);
  assert.equal(tui._selected().name, 'codex'); // last row
  ...
  assert.equal(tui._selected().name, 'claude-b');
  tui._key('up');
  assert.equal(tui._selected().name, 'claude-a');

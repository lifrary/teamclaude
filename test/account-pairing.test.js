...
test('same-person entries in different organizations keep their own tokens through a refresh', async () => {
  // Canonical identities must be distinct; entry IDs pair each live account
  // with its own configuration row even when the display names agree.
  const { config, am } = await startup([
    ...
    { name: 'user@example.com', type: 'oauth', accountUuid: 'uuid-a', orgUuid: 'org-b', accessToken: 't', refreshToken: 'r', expiresAt: 1_000 },
  ]);

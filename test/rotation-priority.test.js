...
// Make an account unavailable so canonical selection skips it.
function exhaust(am, idx) {
  am.accounts[idx].status = 'exhausted';
}

test('canonical comparator preserves config order for an all-unknown tie', () => {
  const am = new AccountManager([oauth('a'), oauth('b'), oauth('c')], 0.98);
  assert.equal(am._selectBest().name, 'a');
});

test('canonical comparator honors lower explicit priority', () => {
...
test('canonical comparator breaks a priority tie by measured weekly reset', () => {
  const am = new AccountManager([
    oauth('a', { priority: 0 }),
    oauth('b', { priority: 0 }),
  ], 0.98);
  const now = Date.now();
  am.accounts[0].quota.unified7d = 0.5; am.accounts[0].quota.unified7dReset = now + 5000;
  am.accounts[1].quota.unified7d = 0.5; am.accounts[1].quota.unified7dReset = now + 1000;
  assert.equal(am._selectBest().name, 'b');
});

test('canonical comparator skips an unavailable higher-priority account', () => {
  const am = new AccountManager([
    oauth('a', { priority: 1 }),
    oauth('b', { priority: 2 }),
  ], 0.98);
...
test('common case: all-equal priority keeps canonical cold-start warm-up active', () => {
  const am = new AccountManager([oauth('a'), oauth('b'), oauth('c')], 0.98);
  am.currentIndex = 2;
  assert.equal(am.getActiveAccount().name, 'a');
});

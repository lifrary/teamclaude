...
test('a pending switch follows a unique stable ID across a remote poll', async (t) => {
  const { tui, am, seen } = await makeSession(t);
  const accounts = statusFixture().accounts.map((account, index) => ({ ...account, id: `seat-${index}` }));
  am.applyStatus(statusFixture({ accounts }));
  tui._key('s');
  tui._key('down');
  am.applyStatus(statusFixture({ accounts: [accounts[1], accounts[0]] }));
  tui._key('enter');
  await logged(tui);
  assert.deepEqual(JSON.parse(seen.at(-1).body), { account: 'bravo' });
  assert.match(tui.log[0].msg, /Switched/);
});

test('a pending remote switch cannot cross IDs through a reused name', async (t) => {
  const { tui, am, seen } = await makeSession(t);
  const account = { ...statusFixture().accounts[0], id: 'original-seat' };
  am.applyStatus(statusFixture({ accounts: [account] }));
  tui._key('s');
  am.applyStatus(statusFixture({ accounts: [{ ...account, id: 'replacement-seat' }] }));
  assert.equal(tui._selected(), null, 'rendering must not silently select the replacement');
  tui._key('enter');
  await logged(tui);
  assert.match(tui.log[0].msg, /no longer listed/i);
...
test('a pending remote switch refuses ambiguous stable IDs', async (t) => {
  const { tui, am, seen } = await makeSession(t);
  const account = { ...statusFixture().accounts[0], id: 'seat' };
  am.applyStatus(statusFixture({ accounts: [account] }));
  tui._key('s');
  am.applyStatus(statusFixture({ accounts: [account, { ...account, name: 'other' }] }));
  tui._key('enter');
  await logged(tui);
  assert.match(tui.log[0].msg, /no longer listed/i);
  assert.deepEqual(seen, []);

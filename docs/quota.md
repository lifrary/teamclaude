...
Observed quota is persisted to `<config>.state.v2.json` next to the config, so rotation state survives a restart. Stale windows are discarded automatically. Older state files are read-only migration inputs, never parallel write targets.
...
The optional quota probe also fills missing tier metadata on its first successful refresh.

## Quota probe

Quota probes use the maintenance coordinator and start immediately when enabled
at startup. As zero-spend reads they still run when inference admission refuses
an exhausted, paused or saturated account. Disabled accounts with valid tokens
remain monitored, without refreshing their OAuth lineage from the probe.
Credentials belonging to a third-party backend or Codex go only to that
provider's own quota reader, never to Anthropic.

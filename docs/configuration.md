...
Credential-free runtime state is written to `<config>.state.v2.json`, with strict
validation on reads and writes. Older `<config>.state.json` and
`<config>.quota.json` files are read-only migration inputs when no v2 state exists.
Deleting state discards learned quota, counters and the accepted warm-up template;
credentials remain in the config.
...
| `activeWarmup` / `warmupIntervalMs` | Capture-and-replay quota measurement, enabled by default with a five-minute interval; interval `0` is startup-only. Uses an accepted request shape and spends a minimal message |
| `tokenRefreshIntervalMs` | Sequential idle OAuth keep-alive sweep, default five minutes; `0` disables it |
| `overloadFallbackModel` | Optional same-account fallback for Opus 529 or pre-header timeout; `null` disables fallback. Does not fan overload out across the fleet |
| `transientRetries` | Bounded internal retries for transient connection failures; default `1` |
...
| `accounts[].priority` | Rotation preference, lower = preferred. Unset/`null` sorts after every finite rank; ties use the quota ordering |

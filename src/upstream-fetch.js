...
function directFetch(url, opts, timeoutMs) {
  ...
  return fetch(url, { ...opts, signal: ctrl.signal }).then(
    ...
    (err) => { clearTimeout(timer); opts.signal?.removeEventListener('abort', onAbort); throw err; },
  );
}
...
function proxiedFetch(url, opts, sx, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    ...
    onAbort = () => req.destroy(opts.signal?.reason);
    if (opts.signal?.aborted) onAbort();
    else opts.signal?.addEventListener('abort', onAbort, { once: true });
    const timer = setTimeout(() => req.destroy(headersTimeoutError(timeoutMs)), timeoutMs);
    timer.unref?.();
    req.once('error', (err) => { clearTimeout(timer); opts.signal?.removeEventListener('abort', onAbort); reject(err); });
